...
- 当前整体处于 🔄 实施中，**D0（基座）、D1（Agentic Workflow 基座）、D3（分层规则纪律）与 D5（Playbook + 子代理）已实施完成**，D2/D4/D6 尚未开始编码。
...
### D4 SOP 标准作业（📝 | 前置：D1 | 后置：D5）

| 子任务 | 内容（一行） | 章节 | 状态 |
|---|---|---|---|
| D4-1 | `SopAsset` + `SopRegistry`（复用 AgentAssetCore frontmatter 解析 + 热加载，扫 `assets/sop/`） | §3.2 | ⬜ |
| D4-2 | 6 份 SOP 资产：10-release / 20-migration / 30-build-verify / 40-approach / 50-commit / 60-ai-conduct（编号步骤：操作 + 判定 + 产出/出错处理） | §3.2 | ⬜ |
| D4-3 | 摘要常驻注入 Source（importance 分级，走预算裁剪）+ 双权威源头注（AGENTS.md / prompts） | §3.2 | ⬜ |
...
| 子任务 | 内容（一行） | 章节 | 状态 |
|---|---|---|---|
| D6-1 | spec-check.sh 配套性触发：新模块 + 新增 .kt 判定（只校验本次触碰） | §3.4 | ⬜ |
| D6-2 | 评审状态行校验（📝 草案 / ✅ 已评审 / 已实施） | §3.4 | ⬜ |
| D6-3 | SOP 同步提示：精确映射表（AGENTS.md ↔ sop/10-50、prompts 行为规则 ↔ sop/60-ai-conduct） | §3.4 / §3.2 | ⬜ |
| D6-4 | githooks 接入（warning 级、不阻断）+ 无误报验证 | §3.4 | ⬜ |
