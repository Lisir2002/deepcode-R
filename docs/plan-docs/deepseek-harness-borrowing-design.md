...
MiniMe-core 是 Android 端 AI 编程工具，已具备 Agent 循环、工具注册与权限、PRoot 容器 + 远程 SSH 执行、MCP 客户端/服务器、消息分库持久化、上下文压缩等能力。DeepSeek Harness（下称 DSH）是 DeepSeek 开源的 Agent 运行时框架，沉淀了大量 Agent 工程化护栏与能力抽象。

### 1.2 目标

深度阅读 DSH 源码与子系统文档，盘点其优势能力与架构，逐项评估能否借鉴到 MiniMe-core，输出分优先级的落地路线，避免"无中生有"式照搬。
...
DSH 最值得 MiniMe-core 吸收的**不是"插件框架"这一层**（Android 端不适用），而是它沉淀在工具链路上的**工程化护栏**——canonical 输出契约、读前必改 + CAS、结构化超时、重复调用提醒、事件化压缩。这些与已落地的流式归一化（`DeltaAccumulator`）同源同向，是本项目最该补齐的短板。
