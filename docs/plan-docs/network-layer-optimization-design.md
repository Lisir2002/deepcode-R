...
**为什么分场景**：优化项的收益排序在不同网络形态下会反转——C1/C5 在代理/弱网下收益远超 P1，而 P1 在任何场景都有解析收益。因此落地前先建立基准，**用数据决定第 4 节的落地顺序**，而不是按直觉排优先级。

网络形态 × 链路两条轴构成矩阵，每格跑 N 次（建议 ≥5）取中位数：

| 网络形态 | 说明 |
|---|---|
| A 直连 | 无代理，正常 Wi-Fi / 蜂窝 |
| B 代理 | mihomo mixed-port 代理链路（真机默认形态，多一跳） |
| C 弱网 | 限速 + 丢包模拟（如 1Mbps / 5% 丢包） |

| 链路 | 覆盖点 |
|---|---|
| 非流式 | 完整响应 DTO / JsonObject 解析 |
| 流式 | SSE 逐 token 主路径 |
| 流式 + 工具 | 工具调用（含 Anthropic `partial_json` 完整累积路径） |

### 5.2 指标口径

| 指标 | 定义 | 测量手段 |
|---|---|---|
| TTFT | 请求发出 → 首个内容块到达并解析完 | 应用内打点（时间戳） |
| TOKT | 首块后 → 流结束的 token/秒 或 每秒解析块数 | 应用内打点 |
| 解析耗时 | 单行 SSE 解析耗时（与网络解耦的自测） | 独立 benchmark / Systrace |
| 分配 / GC | 解析过程分配字节与 GC 次数 | Android Memory Profiler |
| 连接复用 | 连接池命中率、实际握手次数 | OkHttp `EventListener` / 日志 |

### 5.3 执行协议

- 固定 prompt（固定体积系统提示 + 固定历史，三家各一档）与固定模型，保证可复现。
- 每「场景 × 链路」跑 ≥5 次取中位数，剔除异常抖动。
- 优化前后各测一遍，输出对比表（优化前 / 优化后 / Δ），重点看 TTFT 与解析耗时两列。
- 用对比结果回填第 4 节：若 B/C 场景下 C1/C5 收益显著大于 P1，则调整实施先后。

### 5.4 判定

- **回归**：三家 provider 各跑 非流式 + 流式 + 工具调用 三条链路，保持通过。
- **兼容**：未知字段 / 异常 SSE 行 / 转义字符（`\n \" \uXXXX`）的容错测试保持通过。
- **弱网回退**：C3 DNS 缓存失效、C5 直连失败必须可回退到系统解析 / 代理，不得影响主链路。
