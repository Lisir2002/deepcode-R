...
| # | 绑定点 | 代码位置 | 影响 |
|---|---|---|---|
| P1 | **ABI 单架构打包** | [app/build.gradle.kts](file:///workspace/app/build.gradle.kts#L141) `ndk { abiFilters += "arm64-v8a" }` | x86_64 宿主模拟器**安装即失败**（缺 x86_64 so） |
| P2 | **targetSdk 锁定 28** | [app/build.gradle.kts](file:///workspace/app/build.gradle.kts#L127-L129) | PRoot 需在 app 可写目录执行二进制（Android 10+ W^X）。**在模拟器上同样成立，不是额外负担** |
| P3 | **容器 rootfs 架构资产** | [ContainerInstaller.kt](file:///workspace/app/src/main/java/com/mini/me_core/feature/agent/domain/container/ContainerInstaller.kt#L19-L25) 已留双容器架构设计；`_x86Assets` 已按旧决策删除 | 容器/终端运行期能力 |

**关键利好**：执行后端**已经是可插拔的**——

- [DelegatingFileAccess.kt](file:///workspace/app/src/main/java/com/mini/me_core/feature/workspace/domain/DelegatingFileAccess.kt) 按执行模式自动分发本地/远程文件访问；
...
- **产物命名**：由 `deepcode-arm64-<tag>.apk` 调整为 `deepcode-<tag>.apk`（不再暗示单架构）；CI 校验清单相应改为校验 `lib/` 同时含 `arm64-v8a` 与 `x86_64`（见 [docs/ci-release.md](file:///workspace/docs/ci-release.md) 扩展）。
...
- 复用现有 [DelegatingFileAccess.kt](file:///workspace/app/src/main/java/com/mini/me_core/feature/workspace/domain/DelegatingFileAccess.kt) 分发骨架，新增一条「无容器」路由即可。
