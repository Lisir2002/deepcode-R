# MiniMe-core 设计主计划（Design Master Plan）
...
| D6 Spec 规范驱动 | ✅ 已实施 | 2026-08-26 | `spec-check.sh` 独立预检（配套性触发：新增非 test 路径 .kt/新模块阻断提示补 design 文档，`git diff --cached` 只校验本次触碰；`*-design.md` 两字段校验：`评审状态` 行存在+值合法（📝 草案/✅ 已评审/已实施，纯值不带括号注释/组合态，存量不迁移）阻断，`✅ 已评审` 时 `> 评审结论：` 行建议必填缺失 warning 不阻断；SOP 同步提示：AGENTS.md ↔ sop/10-50、prompts 行为规则 ↔ sop/60-ai-conduct，warning 级不阻断）+ `.githooks/pre-commit` 调用 + `docs/plan-docs/README.md` 规范补充 + `prompts/40-approach.md` 追加「重大改动先出设计文档」+ feature-dev 剧本「设计文档」阶段联动（gates=approval）；无误报验证通过（纯文档/test 路径 .kt 不误报；新增 main .kt/状态行非法正确阻断；评审结论缺失仅 warning）；`assembleDebug` + `testReleaseUnitTest` 全绿 |
| D7 | ⬜ | — | — |
