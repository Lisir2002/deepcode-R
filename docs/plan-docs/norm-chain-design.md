...
**设计**（深入讨论定稿）：
1. **分层级别（四级）**——① **全局**（用户设备级，`~/.minime/global-rules.md`）② **项目**（`AGENTS.md`，已有，权威源）③ **工作区**（工作区根/容器内 `minime` 目录的 `workspace-AGENTS.md`，对特定项目/工作区注入差异化规则）④ **模块**（`feature/<module>/AGENTS.md`，子目录级规则，仅对该模块相关任务生效）。
