...
MiniMe-core 默认使用内置的 Alpine rootfs 通过 PRoot 运行 Linux 容器。如果需要不同的发行版或预装特定工具链的环境，可以导入自定义 rootfs 镜像。
...
- 首次使用自定义镜像时，MiniMe-core 会解压 rootfs 到 App 私有目录，耗时取决于镜像大小
