...
- **执行注入**：命中则走 `SkillExecutor.execute`（PROMPT 注入正文、SCRIPT 走既有 ZTH 审批；**MCP 包装技能不参与自动触发**——已降级为别名，由模型按需直接调用绑定 MCP 工具），输出以「【系统·自动触发技能…】」内容块**合并进最后一条用户消息**（技能输出在前、用户请求在后），保证模型必然以该消息为当前任务并读取其中规则；同时向 UI 推送 `AutoTriggered` 事件落库工具卡片，用户可直观看到「某技能被自动触发并加载」的结果。SCRIPT 自动触发执行前仍弹确认卡（审批卡标题带【自动触发】标记，安全审批链路不变），用户拒绝/超时降级为文本注入不阻塞主流程；**仅成功执行后才标记会话级去重**，失败/被拒可重试。
- **注入权威性加固**：自动触发技能的结果若仅以「独立 UserMessage」注入首轮上下文，轻量/事实问答类模型会把它当作「无关历史」忽略，只按最后一条用户消息作答（实测：永远只按用户消息给结果，技能里的规则文档不生效）。因此采用**合并进最后一条用户消息**的注入方式（`技能输出\n\n【本次用户请求】\n用户请求`），模型必然读到；同时系统提示词末尾追加一段【系统】指令，声明这些技能输出为**必须参考并落实的权威上下文**（规则/约束/检查项/参考资料）。双重保障确保模型真正落实技能输出，而不只是"供参考"；合并方式也避免了「连续多条 User 消息」触发 Anthropic 等要求 user/assistant 交替的 API 报错。
