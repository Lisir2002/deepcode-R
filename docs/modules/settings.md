...
设置页以 `SkillsSection` 承接技能管理：`SkillsViewModel` 负责技能启用/禁用/卸载（agent 侧 `SkillStateRepository` 的 Room 状态），`SkillsScreen.kt` 为列表 UI。技能加载/执行语义由 agent 模块承载，见 [agent.md §3.6](./agent.md#36-技能skill) 与 [skill-collaboration-design](../plan-docs/skill-collaboration-design.md)。

**作用域分级在设置侧的体现**（多 Agent 演进，📝 草案）：技能按 `SkillScope`（GLOBAL/COMMON/AGENT）分级，见 [skill-collaboration-design §4.5](../plan-docs/skill-collaboration-design.md#45-技能作用域分级skill-scope)：
- `GLOBAL 全局`：技能中心**不出开关**（不可关闭，系统级强制激活）；
- `COMMON 通用`：技能中心**展示用户自定义开关**（默认启用，可关）；
- `AGENT`：**仅当当前 agentType 匹配时才可见/可开关**，非当前 agent 的技能在设置页可按 agent 维度分组展示或隐藏。

即设置侧只需暴露「能关」的：GLOBAL 静默内置、COMMON 提供开关、AGENT 按 agent 过滤；实际的按作用域动态注册/回收由 `SkillToolBindingManager` 在 agent 激活时执行。
...
| `SkillsViewModel` | 技能中心：`enable/disable/uninstall` 写 agent 侧 `SkillStateRepository` 的 Room 启用状态；按 `SkillScope` 分发开关可见性（GLOBAL 不可关 / COMMON 可关 / AGENT 按 agentType 过滤） |
