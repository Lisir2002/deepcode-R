...
> 模块路径：`app/src/main/java/com/mini/me_core/feature/settings/`；维护规则：本模块代码变更必须同步更新本文档

## 1. 模块定位

负责 MiniMe-core 的**全部应用设置**：AI Provider（供应商/API Key/模型）管理与模型能力元数据解析、日志（等级/查看器/筛选/实时尾随）、主题、后台保活、MCP 服务器、权限规则、容器 Profile 与执行模式（本地 PRoot / 远程 SSH）、安全设置（凭据加密状态/密钥轮换/ZTH 档位）、技能中心、关于/更新等。
