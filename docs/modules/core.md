...
| `core/worker/` | WorkManager 任务：`AuditPurgeWorker`（审计清理） |
| `di/` | Hilt 模块：`AgentModule`、`BackupModule`、`RepositoryModule` |
| `AIEditorApp.kt` / `MainActivity.kt` | 应用入口：初始化核心服务、启动导航 |

## 3. 核心架构与主流程

- **数据库**：数据层为 **SQLDelight V2 六库拓扑**（`datalayer/`，见 [数据层章节](../plan-docs/*-design.md) 与 AGENTS.md）；旧 Room 数据层（世代0 单巨库 `LegacyAgentDatabase`、世代1 按域拆 5 库、全部 DAO/`@Entity`/迁移基建）已完全移除。业务读写全走 `datalayer/repository` 门面（`AgentRepository` + 泛型 store），旧数据经 `core/db/V1toV2FullMigrator`（纯 SQLite，幂等、只跑一次）在启动时一次性搬入 V2。
- **数据注册表**：`core/data/DataRegistry` 枚举全应用数据域（V2 表 + DataStore 目录），提供 `snapshotAll/restoreAll/pack/unpack`；V2 库的表转储经 `datalayer/backup/SqlDelightDataProvider`（基于 SQLDelight driver forEach/PRAGMA）。备份/恢复/自动迁移统一经注册表全量覆盖（见 backup 模块文档）。
...
- `V1toV2FullMigrator`：被 `AIEditorApp` 启动调用（幂等一次性移植，纯 SQLite 直读旧 Room 域库文件）。
...
- **v0.0.0.1-rc3（2026-09-03）**：自愈幂等加固。
  - `SchemaSelfHealer.healTable` 在 `ALTER TABLE ... RENAME TO ..._legacy` 前，先 `DROP TABLE IF EXISTS ${table}_legacy`（`try/catch` 包裹、失败不阻断）：清理上次自愈在 RENAME 后未 DROP 就中断所遗留的临时表，避免本次 RENAME 撞名失败、永久锁死后续自愈。
