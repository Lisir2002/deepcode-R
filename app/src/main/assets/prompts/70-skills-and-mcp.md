...
- **取用方式按类型分流（唯一约定，勿混淆）**：
  - **PROMPT 指令技能**：用 `loadSkill` 传入技能名，拿到 `SKILL.md` 完整正文（含依赖指令），严格按正文行动。
  - **SCRIPT 脚本技能**：用 `runSkillScript` 执行其入口脚本（容器沙箱内运行，执行前会征求用户确认，参数经 `args` 传入并注入为 `SKILL_ARG_*` 环境变量）。若只需阅读其 `SKILL.md` 正文，可用文件工具直接读取 `~/.rcodecore/skills/<name>/SKILL.md`。
  - **MCP 包装技能**：已降级为别名——直接调用其绑定的 MCP 工具（如 `mcp__server__tool`），不再经技能系统执行。
- **自动触发（autoTrigger）**：声明了 `auto_trigger: true` 的技能，系统会在新任务到来时作为自动化流程的一环智能判断是否命中其触发条件，命中即自动加载/执行并把输出以「【系统·自动触发技能…】」消息注入本轮上下文。看到该消息即视为技能正文/执行结果已就位，直接依其内容行动，不要重复 `loadSkill`/`runSkillScript`；未命中则照常按下方流程手动取用。
...
- 使用流程：
  ...
  2. PROMPT 技能 → 调用 `loadSkill` 拿完整正文；SCRIPT 技能 → 调用 `runSkillScript` 执行入口脚本。
