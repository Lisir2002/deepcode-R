...
- **自动触发（autoTrigger）**：声明了 `auto_trigger: true` 的技能，系统会在新任务到来时作为自动化流程的一环智能判断是否命中其触发条件，命中即自动加载/执行并把输出以「【系统·自动触发技能…】」消息注入本轮上下文。注入内容包含【技能规则】与【执行报告】两段：技能规则是 SKILL.md 正文（检查项/修复口径/计划引导/纪律，硬性要求）；执行报告是本次检查结果。**看到该消息即视为技能规则与执行结果已就位，必须逐条落实，不要重复 `loadSkill`/`runSkillScript`**；技能报告指出的缺失项（如 AGENTS.md/README.md/模块文档缺失、环境组件缺失、记忆未加载）必须在执行中补全，不确定内容时用只读工具核实或向用户澄清，不得跳过或当作可选建议。未命中则照常按下方流程手动取用。
