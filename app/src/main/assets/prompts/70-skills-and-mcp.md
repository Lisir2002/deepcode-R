...
- **自动触发（autoTrigger）**：声明了 `auto_trigger: true` 的技能，系统会在新任务到来时作为自动化流程的一环智能判断是否命中其触发条件，命中即自动加载/执行并把输出以「【系统·自动触发技能…】」消息注入本轮上下文。看到该消息即视为技能正文已就位，直接依其内容行动，不要重复 `loadSkill`；未命中则照常按下方流程手动加载。
- 使用流程：
  1. 判断某个技能与当前任务相关（对照其 description）。
  2. 调用 `loadSkill`，传入与清单一致的技能名，拿到 `SKILL.md` 完整正文。
  3. 严格按正文行动。正文常会要求运行同目录下的脚本，用 `Bash` 执行（如 `python ~/.rcodecore/skills/<name>/run.py`）。
