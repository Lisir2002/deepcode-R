...
- AI 配置目录固定为 `~/.deepcode`，可用文件工具或 `Bash` 直接访问；它映射到 Android 宿主私有目录 `filesDir/deepcode`，不在 rootfs 内，容器重装不会清空。
- 用户若拥有 Android root 权限，可绕过 DocumentsProvider 直接从宿主访问 App 私有目录：`/data/data/com.core.deepcode/files/`（部分系统显示为 `/data/user/0/com.core.deepcode/files/`）。其中 `projects/` 是本地工作区根，`deepcode/` 对应容器内 `~/.deepcode`。
- `Bash` 的当前目录已经是 `~/workspace`，相对路径都基于该项目根目录解析。
- `~/.deepcode/tool-output/...` 是工具完整输出日志目录，可直接用 `readFile` 分段读取。
...
## Git 工程化工具
- `gitops`：把仓库 Git 规范、提交纪律、发版流程、版本日志生成等经验沉淀为 Agent 可调用的工具。**参数 `action` 必选**，取值与 DeepCore-Code `scripts/gitops/gitops.sh` CLI 对齐：
...
1. **一次性环境准备（只跑一次）**
   - 调用 `ensure_android_env()`（不传参数，按默认值）。它完成：
     ...
     - **关键**：在 aarch64 架构下，自动确保 `qemu-user-static` 装好并运行 `deepcode-wrap-android-buildtools`，把 Build-Tools 下所有 x86_64 ELF 转成 `qemu-x86_64 <original_bin> "$@"` 的同名 shell wrapper，**从根上消除 Exec format error**
     - 把 JAVA_HOME / ANDROID_HOME / PATH 追加写入 `~/.deepcode/env.sh`，后续 Bash / terminal 登录自动 source
