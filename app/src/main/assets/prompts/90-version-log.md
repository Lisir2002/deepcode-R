...
- 库文件名、容器目录、APK 名沿用 rc11（`mini_me_core_agent_v3.db` / `/root/.mini_me_core` / `mini_me_core-<版本>.apk`），本版未改动。

## 0.0.0.1-rc11 (2026-09-04)

### TYPE
prerelease-rc / feat (full brand iteration: MiniMe-core)

### IMPACT
- 应用名 / 包名 / 代码包结构 / 桌面图标全量迭代；AI 工作流无工具、无 schema、无接口语义变化。
- **包名（applicationId）`com.R.codecore` → `com.core.mini_me_core`**：对应用层是「全新安装」，
  旧包私有目录（`/data/data/com.R.codecore/`）数据不可见。用户侧数据找回走「外部安全备份 → 导入恢复」。
- AI 侧影响：凡硬编码包名的路径 / 库名 / 工具脚本引用，一律改新名（见下）。

### DATA / SCHEMA
- **无 schema 版本变化、无表结构变化、无 `.sqm` 迁移**。`schema.version` 不变。
- agent 库文件名：`rcodecore_agent_v3.db` → `mini_me_core_agent_v3.db`（`LibName.AGENT.fileName`）。
  因包名变更导致数据目录本就是全新的，本次换名**无额外数据损失**；
  `LibName.fileName` 的数据契约依然生效——此后改文件名即清空该库历史，禁止再用「改文件名换库」规避问题。
- 容器运行时目录：`/root/.rcodecore` → `/root/.mini_me_core`（rootfs 内无该字符串的硬编码引用，宿主侧解压路径由代码控制，安全）。
- APK 产物名：`rcodecore-<版本>.apk` → `mini_me_core-<版本>.apk`（CI workflow 的 Rename / Upload / Release 文案同步）。

### AI ACTION
- 排查容器 / 数据库问题时注意路径与库名已迭代：容器内 home 为 `/root/.mini_me_core`，agent 库为 `mini_me_core_agent_v3.db`。
...
- 历史包名链（事实记录，勿回退）：`com.aicodeeditor` → `com.deep.rcode` → `com.R.codecore` → `com.core.mini_me_core`。
