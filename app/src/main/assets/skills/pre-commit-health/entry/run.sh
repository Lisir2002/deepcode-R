...
if [ -z "$CHANGED" ]; then
  ...
  if [ -d "$ROOT/app/src/main/java/com/mini/me_core/feature" ]; then
    CHANGED="$(find "$ROOT/app/src/main/java/com/mini/me_core/feature" -type f -name '*.kt' | sed "s|^$ROOT/||")"
  fi
fi
...
mods=$(printf '%s\n' "$CHANGED" | while read -r f; do
  m=$(printf '%s\n' "$f" | sed -n 's|.*/com/mini/me_core/feature/\([^/]*\)/.*|\1|p')
  [ -n "$m" ] && printf '%s\n' "$m"
done | sort -u)
if [ -n "$mods" ]; then
  for m in $mods; do
...
if [ -d "$ROOT/docs/modules" ]; then
  for doc in "$ROOT"/docs/modules/*.md; do
    ...
    if [ ! -d "$ROOT/app/src/main/java/com/mini/me_core/feature/$n" ]; then
...
if has_pref '^app/src/main/assets/prompts/' || has_pref '^app/src/main/java/com/mini/me_core/feature/.*/.*Tool\.kt' || has_pref '(AgentTool\.kt|StatefulAgentWorkflow\.kt)'; then
  echo "⚠️  [W-1] 变更涉 AI 工作流/工具：检查 assets/prompts/ 与 assets/docs/ 是否需同步（prompts/docs 资产同步纪律）"
  WARNINGS=$((WARNINGS+1))
fi

# ================= 建议项 W-2：模块文档反映行为变化 =================
if has_pref '^app/src/main/java/com/mini/me_core/'; then
...
if $IS_ANDROID; then
  if has_pref '^app/src/main/java/com/mini/me_core/feature/'; then
    suggested_type="feat"
