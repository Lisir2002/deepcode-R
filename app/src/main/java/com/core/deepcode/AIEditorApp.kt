...
import com.core.deepcode.core.util.AILogger
...
import com.core.deepcode.feature.credentials.data.GitCredentialsFileSync
...
import com.core.deepcode.feature.settings.data.repository.resolveSshConfigOrNull
...
import com.core.deepcode.datalayer.engine.ConnectionPool
...
class AIEditorApp : Application() {
    ...
    private companion object {
        ...
        const val DIAG_PREFS = "deepcode_diag"
    ...
    @Inject
    lateinit var mcpServerManager: com.core.deepcode.feature.agent.domain.mcp.server.McpServerManager
    ...
    @Inject
    lateinit var executionModeRepository: com.core.deepcode.feature.settings.data.repository.ExecutionModeRepository
    ...
    @Inject
    lateinit var workspaceRepository: com.core.deepcode.feature.workspace.data.repository.WorkspaceRepository

    /** 远程主机配置仓库：冷启动按 activeConnectionId 从 Room 单源取主机 + 凭据（密码/私钥）。 */
    @Inject
    lateinit var remoteRepository: com.core.deepcode.feature.workspace.domain.repository.RemoteRepository

    /** 容器 profile 仓库：冷启动按 active profile 拿 RootfsSource.RemoteSsh 的 workspacePath。 */
    @Inject
    lateinit var containerSettingsRepository: com.core.deepcode.feature.settings.data.repository.ContainerSettingsRepository
    ...
    @Inject
    lateinit var connectionPrewarmer: com.core.deepcode.core.network.ConnectionPrewarmer
    ...
    @Inject
    lateinit var scheduleScheduler: com.core.deepcode.feature.agent.domain.schedule.ScheduleScheduler
    ...
    override fun onCreate() {
        ...
        // 启动即把内置提示词全量释放到 ~/.deepcode/prompts/（覆盖式，随 App 升级更新）；
        // 用户自定义覆盖放在 ~/.deepcode/prompts.custom/，同名即覆盖，不被升级覆盖。
        appScope.launch {
        ...
        // 启动即把 Room 凭据 + DataStore 署名落盘到容器持久挂载（/root/.deepcode），
        ...
        appScope.launch {
            ...
            runCatching {
                ...
                if (mode == com.core.deepcode.feature.settings.data.repository.ExecutionMode.REMOTE_SSH) {
    ...
    /**
     * 读取 assets/docs 下所有内置文档，通过 SSH exec 同步到远程 ~/.deepcode/docs/。
     * 远程模式下 AI 查阅 ~/.deepcode/docs/ 的设置说明文档时，需要这些文件存在于远程服务器。
