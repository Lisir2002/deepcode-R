 ...
 * 供上层选择注入给模型）。新增护栏仅 `@IntoSet` 注册即可挂载，对齐 [com.core.deepcode.feature.agent.domain.hook.HookDispatcher]
 * 的 multibinding 汇集模式。
 *
 * 链上护栏与执行顺序（设计定稿）：权限检查 → 文件观察 → 危险命令 → 超时钳制；
 * 现有护栏归属（权限在 workflow 层、危险命令/超时钳制在 [com.core.deepcode.feature.agent.domain.tool.container.ExecuteCommandTool]
