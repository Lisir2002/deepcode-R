...
import com.core.deepcode.feature.agent.domain.container.friendlySshError
import com.core.deepcode.feature.workspace.data.repository.WorkspaceRepository
import com.core.deepcode.feature.workspace.domain.WorkspacePathMapper.Companion.CONTAINER_ROOT
...
class RemoteSftpFileAccess @Inject constructor(
    ...
    override fun copyToLocal(path: String): File {
        val remote = toRemotePath(path)
        val tempFile = File.createTempFile("deepcode_remote_", ".copy").apply { deleteOnExit() }
        return runCatching {
            // base64 解码到本地临时文件
            val b64 = execSync("base64 ${shellQuote(remote)} 2>/dev/null")
