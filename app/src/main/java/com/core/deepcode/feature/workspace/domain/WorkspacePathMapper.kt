package com.core.deepcode.feature.workspace.domain
...
class WorkspacePathMapper @Inject constructor(
    ...
    /** AI 配置目录在宿主上的根（容器内 `/root/.deepcode` 即此目录，独立于 rootfs）。 */
    private fun deepcodeRoot(): File = containerInstaller.deepcodeDir

    /**
     * 把 AI 提供的路径解析为宿主真实文件。兼容以下写法：
     * - 容器绝对路径 `~/workspace[/…]`（或展开后的 `$HOME/workspace[/…]`）→ 映射到宿主工作区；
     * - 容器绝对路径 `/root/.deepcode[/…]` → 映射到宿主 AI 配置目录（skill / mcp.json，独立于 rootfs）；
     * - 其它容器绝对路径 `/etc/…`、`/root/…` → 映射到 rootfs 内对应文件（容器系统文件）；
     * - 相对路径 `src/Main.kt` → 挂到宿主工作区根下；
     *
     * `/root/.deepcode` 必须先于通用 `/`→rootfs 规则匹配，否则会落到 rootfs 内的临时副本（升级即丢）。
     */
     ...
     * 把宿主路径还原为容器路径：
     * - 位于工作区内 → `~/workspace/…`；
     * - 位于 AI 配置目录内 → `/root/.deepcode/…`；
     * - 位于 rootfs 内 → 去掉 rootfs 前缀的容器绝对路径（如 `/etc/apk/repositories`）；
     * - 其余原样返回（极少出现）。
     *
     * 工作区在 `filesDir/projects`、AI 配置在 `filesDir/deepcode`、rootfs 在 `filesDir/rootfs`，三者互不重叠，
     * 判断顺序无歧义。
     */
    fun toContainerPath(hostPath: String): String {
        val rootPath = hostRoot().absolutePath
        val deepcodePath = deepcodeRoot().absolutePath
