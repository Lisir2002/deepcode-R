...
import com.core.deepcode.feature.agent.domain.prompt.AgentAsset
...
import com.core.deepcode.feature.agent.domain.skill.SkillRepository
...
class ExtensionLoader @Inject constructor(
    ...
    /** 用户扩展根目录 `<deepcode>/ext/`（首次访问建目录）。 */
    val userExtDir: File by lazy {
        File(containerInstaller.deepcodeDir, "ext").also { it.mkdirs() }
    }

    /** 用户命令目录 `<deepcode>/ext/commands/`。 */
