...
class ContainerInstaller @Inject constructor(
    ...
    companion object {
        ...
        /** 从 assets 提取 SOP 标准作业到 ~/.deepcode/sop/（D4-1，内置默认副本，每次启动全量覆盖）。 */
        @Volatile private var sopExtractedSession = false

        /** 从 assets 提取 Playbook 剧本资产到 ~/.deepcode/playbooks/（D5-2，内置默认副本，每次启动全量覆盖）。 */
        @Volatile private var playbooksExtractedSession = false

        /** 从 assets 提取 SOP 标准作业资产到 ~/.deepcode/sop/，使 App 升级后 SOP 随之更新。 */
        fun extractSop(context: Context) {
            if (sopExtractedSession) return
            val destDir = File(File(context.filesDir, "deepcode"), "sop")
            destDir.mkdirs()
            runCatching {
        ...
        /** 从 assets 提取文档到 ~/.deepcode/docs (内置使用指导) */
        fun extractDocs(context: Context) {
            if (docsExtractedSession) return
            val destDir = File(File(context.filesDir, "deepcode"), "docs")
            destDir.mkdirs()
            runCatching {
        ...
        /** 从 assets 提取 Playbook 剧本资产到 ~/.deepcode/playbooks/（D5-2，内置默认副本，使 App 升级后剧本随之更新）。 */
        fun extractPlaybooks(context: Context) {
            if (playbooksExtractedSession) return
            val destDir = File(File(context.filesDir, "deepcode"), "playbooks")
            destDir.mkdirs()
            runCatching {
        ...
        /**
         * 从 assets 提取内置提示词到 ~/.deepcode/prompts/，每次启动全量覆盖，使 App 升级后提示词随之更新。
         *
         * 用户自定义覆盖放在 ~/.deepcode/prompts.custom/（同名即覆盖），本方法不触碰该目录，
         * 故用户重写的片段不会被升级覆盖。参见 [com.core.deepcode.feature.agent.domain.prompt.SystemPromptProvider]。
         */
        fun extractPrompts(context: Context) {
            val destDir = File(File(context.filesDir, "deepcode"), "prompts")
            destDir.mkdirs()
            runCatching {
        ...
        /**
         * 从 assets 提取自定义 git credential helper 到 ~/.deepcode/git-credential-deepcode 并赋可执行位。
         *
         * 经 [LinuxContainerEngine] 的 -b 绑定即容器内 /root/.deepcode/git-credential-deepcode，
         * 由 [LinuxContainerEngine.provisionIfNeeded] 在 `.gitconfig` 里登记为第二个 credential.helper，
         * 排在 `store` 之后兜底未登录（双保险）。helper 详行为见 assets/deepcode/git-credential-deepcode。
         *
        ...
        fun extractCredentialHelper(context: Context) {
            val dest = File(File(context.filesDir, "deepcode"), "git-credential-deepcode")
            runCatching {
                dest.parentFile?.mkdirs()
                context.assets.open("deepcode/git-credential-deepcode").use { input ->
     ...
     * 而本目录承载用户数据，必须跨升级保留。它由 [com.core.deepcode.feature.agent.domain.container.LinuxContainerEngine]
     * 绑定到容器内 `/root/.deepcode`，故 AI / 终端看到的 `/root/.deepcode` 实际落在这里。
     */
    val deepcodeDir: File
        get() = File(context.filesDir, "deepcode")
    ...
    /** 从 assets 提取 SOP 标准作业资产到 ~/.deepcode/sop/（D4-1）。 */
    fun extractSop() = extractSop(context)

    /** 从 assets 提取 Playbook 剧本资产到 ~/.deepcode/playbooks/（D5-2）。 */
    fun extractPlaybooks() = extractPlaybooks(context)

    /** 从 assets 提取 git credential helper 到 ~/.deepcode/git-credential-deepcode 并赋可执行位。 */
