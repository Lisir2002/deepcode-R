package com.core.deepcode.feature.terminal.data.bundle
...
enum class TerminalBundleId(val stableKey: String) {
     ...
     * 安装后提供命令 `deepcode-wrap-android-buildtools` 做自动 wrapper 化；
...
object TerminalBundles {
    ...
    val ALL: List<TerminalBundle> = listOf(
        ...
        TerminalBundle(
            ...
            iconName = "cpu",
            description = "在 aarch64 (ARM64) 手机上运行 Android SDK 中 x86_64 二进制（aapt2 / zipalign / split-select 等）。构建 Android APK 的必需前置。附带 deepcode-wrap-android-buildtools 自动包装脚本。",
            sizeEstimateMb = 35,
            ...
            #   2. 安装 /usr/local/bin/deepcode-wrap-android-buildtools：扫描 ANDROID_HOME/build-tools
            ...
            cat > /usr/local/bin/deepcode-wrap-android-buildtools <<'WRAPPER_SH_EOF'
            #!/bin/sh
            ...
            WRAPPER_SH_EOF
            chmod +x /usr/local/bin/deepcode-wrap-android-buildtools

            if command -v /usr/local/bin/deepcode-wrap-android-buildtools >/dev/null 2>&1; then
              echo "[hook OK] 已安装 deepcode-wrap-android-buildtools。"
            else
              echo "[hook FAIL] deepcode-wrap-android-buildtools 未写成功。"
              exit 1
            fi
            """.trimIndent()
        )
