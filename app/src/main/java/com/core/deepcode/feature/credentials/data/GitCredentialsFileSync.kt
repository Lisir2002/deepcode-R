 ...
 *
 * 宿主目录 `filesDir/deepcode` 即容器内 `/root/.deepcode`（[LinuxContainerEngine] 的 `-b` 绑定，跨 rootfs 升级不丢）。
 * 容器 [buildContainerEnv] 已设 `GIT_CONFIG_GLOBAL=/root/.deepcode/.gitconfig`，`.gitconfig` 里的
 * `[credential] helper = store --file=/root/.deepcode/git-credentials` 由 [provisionIfNeeded] 装容器时
...
class GitCredentialsFileSync @Inject constructor(
    ...
    private companion object {
        ...
        /** 容器内 `.deepcode` 挂载点对应的宿主目录（即文件实际落盘处）。 */
        const val AICODE_DIR_NAME = "deepcode"
        const val CREDENTIALS_NAME = "git-credentials"
    }

    /** 目标宿主目录（容器内 /root/.deepcode 的物理后端）。懒建。 */
    private val deepcodeDir: File get() = File(context.filesDir, AICODE_DIR_NAME).apply { mkdirs() }
    ...
    private fun writeCredentials(creds: List<GitCredential>) {
        ...
        writeAtomically(File(deepcodeDir, CREDENTIALS_NAME), sb.toString())
    }
