...
import com.core.deepcode.core.db.entity.CredentialEncryptionStateEntity
...
class CredentialEncryptor @Inject constructor(
    ...
    private fun decryptV1Legacy(formatted: String): String {
        return try {
            ...
            keyStore.load(null)
            val alias = "deepcode_credential_key"
    ...
    fun isV1KeyAvailable(): Boolean {
        return try {
            val keyStore = java.security.KeyStore.getInstance("AndroidKeyStore")
            keyStore.load(null)
            keyStore.containsAlias("deepcode_credential_key")
        } catch (e: Exception) {
            false
        }
    }
    ...
    private fun com.core.deepcode.datalayer.sqldelight.workspace.Credential_encryption_state.toEntity() = CredentialEncryptionStateEntity(
        id = id.toInt(),
