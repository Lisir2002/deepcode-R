...
import com.core.deepcode.datalayer.sqldelight.workspace.Workspace_file
import com.core.deepcode.datalayer.sqldelight.workspace.Workspace_project
...
class WorkspaceRepository(private val db: WorkspaceDb) {
    ...
    suspend fun getRemoteConnection(id: String): com.core.deepcode.datalayer.sqldelight.workspace.Remote_connections? =
        withContext(Dispatchers.IO) { q.selectRemoteConnectionById(id).executeAsOneOrNull() }

    suspend fun listRemoteConnections(): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_connections> =
    ...
    suspend fun listRemoteMounts(connectionId: String): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_mounts> =
        withContext(Dispatchers.IO) { q.selectRemoteMountsByConnection(connectionId).executeAsList() }

    suspend fun getRemoteMount(id: String): com.core.deepcode.datalayer.sqldelight.workspace.Remote_mounts? =
        withContext(Dispatchers.IO) { q.selectRemoteMountById(id).executeAsOneOrNull() }

    suspend fun listAllRemoteMounts(): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_mounts> =
    ...
    suspend fun listAuditLogs(connectionId: String): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_audit_logs> =
        withContext(Dispatchers.IO) { q.selectAuditLogsByConnection(connectionId).executeAsList() }

    suspend fun listAllAuditLogs(): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_audit_logs> =
        withContext(Dispatchers.IO) { q.selectAllAuditLogs().executeAsList() }

    /** 分页：offset/limit 降序。 */
    suspend fun pageAuditLogs(offset: Long, limit: Long): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_audit_logs> =
        withContext(Dispatchers.IO) { q.selectAuditLogsPage(limit, offset).executeAsList() }

    /** 按连接分页：取最近 limit 条。 */
    suspend fun pageAuditLogsByConnection(connectionId: String, limit: Long): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_audit_logs> =
    ...
    suspend fun filterAuditLogs(
        ...
        sinceMs: Long,
        limit: Long,
    ): List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_audit_logs> =
        withContext(Dispatchers.IO) {
    ...
    suspend fun getEncryptionState(): com.core.deepcode.datalayer.sqldelight.workspace.Credential_encryption_state? =
    ...
    fun observeAllRemoteConnections(): Flow<List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_connections>> =
        q.selectAllRemoteConnections().asFlow().mapToList(Dispatchers.IO)

    fun observeRemoteMountsByConnection(connectionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_mounts>> =
        q.selectRemoteMountsByConnection(connectionId).asFlow().mapToList(Dispatchers.IO)

    fun observeAllRemoteMounts(): Flow<List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_mounts>> =
        q.selectAllRemoteMounts().asFlow().mapToList(Dispatchers.IO)

    fun observeAllAuditLogs(): Flow<List<com.core.deepcode.datalayer.sqldelight.workspace.Remote_audit_logs>> =
