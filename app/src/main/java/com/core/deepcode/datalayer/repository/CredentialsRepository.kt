...
import com.core.deepcode.datalayer.sqldelight.credentials.Cred_connection
import com.core.deepcode.datalayer.sqldelight.credentials.Cred_secret
...
class CredentialsRepository(private val db: CredentialsDb) {
    ...
    suspend fun getGitCredential(id: String): com.core.deepcode.datalayer.sqldelight.credentials.Git_credentials? =
        withContext(Dispatchers.IO) { q.selectGitCredentialById(id).executeAsOneOrNull() }

    suspend fun listGitCredentials(): List<com.core.deepcode.datalayer.sqldelight.credentials.Git_credentials> =
        withContext(Dispatchers.IO) { q.selectAllGitCredentials().executeAsList() }

    suspend fun getDefaultGitCredential(): com.core.deepcode.datalayer.sqldelight.credentials.Git_credentials? =
    ...
    suspend fun findGitCredentialByHost(host: String): com.core.deepcode.datalayer.sqldelight.credentials.Git_credentials? =
    ...
    fun observeAllGitCredentials(): Flow<List<com.core.deepcode.datalayer.sqldelight.credentials.Git_credentials>> =
