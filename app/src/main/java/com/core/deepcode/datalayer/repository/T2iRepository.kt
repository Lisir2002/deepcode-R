...
import com.core.deepcode.datalayer.sqldelight.t2i.T2i_result
import com.core.deepcode.datalayer.sqldelight.t2i.T2i_task
...
class T2iRepository(private val db: T2iDb) {
    ...
    suspend fun getT2iProvider(id: String): com.core.deepcode.datalayer.sqldelight.t2i.T2i_providers? =
        withContext(Dispatchers.IO) { q.selectT2iProviderById(id).executeAsOneOrNull() }

    suspend fun listT2iProviders(): List<com.core.deepcode.datalayer.sqldelight.t2i.T2i_providers> =
        withContext(Dispatchers.IO) { q.selectAllT2iProviders().executeAsList() }

    suspend fun getActiveT2iProvider(): com.core.deepcode.datalayer.sqldelight.t2i.T2i_providers? =
    ...
    suspend fun listT2iModels(providerId: String): List<com.core.deepcode.datalayer.sqldelight.t2i.T2i_provider_models> =
    ...
    fun observeAllT2iProviders(): Flow<List<com.core.deepcode.datalayer.sqldelight.t2i.T2i_providers>> =
        q.selectAllT2iProviders().asFlow().mapToList(Dispatchers.IO)

    fun observeT2iModelsByProvider(providerId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.t2i.T2i_provider_models>> =
