package com.core.deepcode.datalayer.repository
...
import com.core.deepcode.datalayer.sqldelight.agent.Agent_session
import com.core.deepcode.datalayer.sqldelight.agent.SelectAllSessionsWithCount
...
class AgentRepository(private val db: AgentDb) : WakeQueueStore {
    ...
    suspend fun listCheckpoints(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_checkpoint> =
    ...
    fun observeCheckpointsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_checkpoint>> =
        q.selectCheckpointsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeTodoItemsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Todo_items>> =
        q.selectTodoItemsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeFileEditHunksBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.File_edit_hunks>> =
        q.selectFileEditHunksBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeModeSwitchesBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Mode_switch_history>> =
        q.selectModeSwitchesBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeGoalsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_goals>> =
        q.selectGoalsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observePlansBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_plans>> =
        q.selectPlansBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeJobsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_jobs>> =
        q.selectJobsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeSchedulesBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_schedules>> =
        q.selectSchedulesBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeTrajectoriesBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_trajectories>> =
        q.selectTrajectoriesBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observePlaybookRunsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Agent_playbook_runs>> =
        q.selectPlaybookRunsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeAllSkillStates(): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Skill_state>> =
        q.selectAllSkillStates().asFlow().mapToList(Dispatchers.IO)

    fun observePendingWakeItems(): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Wake_queue>> =
        q.selectPendingWakeItems().asFlow().mapToList(Dispatchers.IO)

    fun observeSentinelsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Zth_user_confirmed_sentinels>> =
        q.selectSentinelsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeFuse(scope: String, scopeId: String): Flow<com.core.deepcode.datalayer.sqldelight.agent.Zth_hallucination_fuses?> =
        q.selectFuse(scope, scopeId).asFlow().mapToOneOrNull(Dispatchers.IO)

    fun observeAllFuses(): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Zth_hallucination_fuses>> =
        q.selectAllFuses().asFlow().mapToList(Dispatchers.IO)

    fun observeRejectionAuditsBySentinel(sentinelId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Zth_sentinel_plan_rejection_audits>> =
        q.selectRejectionAuditsBySentinel(sentinelId).asFlow().mapToList(Dispatchers.IO)

    fun observeRestoreLogsBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Zth_l0_soft_compact_restore_logs>> =
        q.selectRestoreLogsBySession(sessionId).asFlow().mapToList(Dispatchers.IO)

    fun observeTelemetryByKind(eventKind: String, limit: Long): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Zth_telemetry_events>> =
        q.selectTelemetryByKind(eventKind, limit).asFlow().mapToList(Dispatchers.IO)

    fun observeAllTelemetry(): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Zth_telemetry_events>> =
    ...
    suspend fun listTodos(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Todo_items> =
    ...
    suspend fun getTodoPageAfter(lastCreatedAtMs: Long, lastId: String, limit: Long): List<com.core.deepcode.datalayer.sqldelight.agent.Todo_items> =
        withContext(Dispatchers.IO) { q.selectTodoPageAfter(lastCreatedAtMs, lastId, limit).executeAsList() }

    suspend fun getTodoPageBySessionAfter(sessionId: String, lastCreatedAtMs: Long, lastId: String, limit: Long): List<com.core.deepcode.datalayer.sqldelight.agent.Todo_items> =
        withContext(Dispatchers.IO) { q.selectTodoPageBySessionAfter(sessionId, lastCreatedAtMs, lastId, limit).executeAsList() }

    suspend fun upsertAllTodos(todos: List<com.core.deepcode.datalayer.sqldelight.agent.Todo_items>) = withContext(Dispatchers.IO) {
        todos.forEach { t ->
    ...
    suspend fun getCheckpointById(checkpointId: String): com.core.deepcode.datalayer.sqldelight.agent.Session_checkpoints? =
        withContext(Dispatchers.IO) { q.selectCheckpointById(checkpointId).executeAsOneOrNull() }

    suspend fun getCheckpointByMessageId(messageId: String): com.core.deepcode.datalayer.sqldelight.agent.Session_checkpoints? =
        withContext(Dispatchers.IO) { q.selectCheckpointByMessageId(messageId).executeAsOneOrNull() }

    suspend fun listCheckpointsForSession(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Session_checkpoints> =
    ...
    suspend fun listCheckpointFileSnapshots(checkpointId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Checkpoint_file_snapshots> =
    ...
    suspend fun listFileEditHunks(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.File_edit_hunks> =
    ...
    suspend fun listModeSwitches(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Mode_switch_history> =
    ...
    suspend fun getCapabilityOverride(providerType: String, modelId: String): com.core.deepcode.datalayer.sqldelight.agent.Model_capability_overrides? =
        withContext(Dispatchers.IO) { q.selectCapabilityOverride(providerType, modelId).executeAsOneOrNull() }

    fun observeCapabilityOverride(providerType: String, modelId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Model_capability_overrides>> =
    ...
    suspend fun listGoals(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_goals> =
    ...
    suspend fun getGoalById(goalId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_goals? =
        withContext(Dispatchers.IO) { q.selectGoalById(goalId).executeAsOneOrNull() }

    suspend fun getActiveGoalBySession(sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_goals? =
        withContext(Dispatchers.IO) { q.selectActiveGoalBySession(sessionId).executeAsOneOrNull() }

    /** 阻塞版（仅供 [runInTx] 事务块内使用，同线程）。 */
    fun getActiveGoalBySessionBlocking(sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_goals? =
    ...
    suspend fun listPlans(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_plans> =
    ...
    suspend fun getPlanById(planId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_plans? =
        withContext(Dispatchers.IO) { q.selectPlanById(planId).executeAsOneOrNull() }

    suspend fun getLatestPlanBySession(sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_plans? =
        withContext(Dispatchers.IO) { q.selectLatestPlanBySession(sessionId).executeAsOneOrNull() }

    /** 阻塞版（仅供 [runInTx] 事务块内使用，同线程）。 */
    fun getLatestPlanBySessionBlocking(sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_plans? =
    ...
    suspend fun getJobById(jobId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_jobs? =
        withContext(Dispatchers.IO) { q.selectJobById(jobId).executeAsOneOrNull() }

    suspend fun listJobs(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_jobs> =
        withContext(Dispatchers.IO) { q.selectJobsBySession(sessionId).executeAsList() }

    suspend fun listRunningJobs(): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_jobs> =
    ...
    suspend fun getScheduleById(scheduleId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_schedules? =
        withContext(Dispatchers.IO) { q.selectScheduleById(scheduleId).executeAsOneOrNull() }

    suspend fun listSchedules(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_schedules> =
        withContext(Dispatchers.IO) { q.selectSchedulesBySession(sessionId).executeAsList() }

    suspend fun getPendingSchedules(): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_schedules> =
    ...
    suspend fun listTrajectories(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_trajectories> =
        withContext(Dispatchers.IO) { q.selectTrajectoriesBySession(sessionId).executeAsList() }

    suspend fun listTrajectoriesByTask(taskId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_trajectories> =
    ...
    suspend fun getTrajectoryAggregate(sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.SelectTrajectoryAggregateBySession =
    ...
    suspend fun getPlaybookRunById(runId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_playbook_runs? =
        withContext(Dispatchers.IO) { q.selectPlaybookRunById(runId).executeAsOneOrNull() }

    suspend fun listPlaybookRuns(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_playbook_runs> =
        withContext(Dispatchers.IO) { q.selectPlaybookRunsBySession(sessionId).executeAsList() }

    suspend fun getLatestPlaybookBySession(sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_playbook_runs? =
        withContext(Dispatchers.IO) { q.selectLatestPlaybookBySession(sessionId).executeAsOneOrNull() }

    suspend fun getLatestPlaybookBySessionAndStatus(sessionId: String, status: String): com.core.deepcode.datalayer.sqldelight.agent.Agent_playbook_runs? =
        withContext(Dispatchers.IO) { q.selectLatestPlaybookBySessionAndStatus(sessionId, status).executeAsOneOrNull() }

    suspend fun listRunningPlaybookRuns(): List<com.core.deepcode.datalayer.sqldelight.agent.Agent_playbook_runs> =
    ...
    suspend fun getSkillState(id: String): com.core.deepcode.datalayer.sqldelight.agent.Skill_state? =
        withContext(Dispatchers.IO) { q.selectSkillStateById(id).executeAsOneOrNull() }

    suspend fun listSkillStates(): List<com.core.deepcode.datalayer.sqldelight.agent.Skill_state> =
    ...
    suspend fun getSkillConversationState(skillId: String, sessionId: String): com.core.deepcode.datalayer.sqldelight.agent.Skill_conversation_state? =
        withContext(Dispatchers.IO) { q.selectSkillConversationState(skillId, sessionId).executeAsOneOrNull() }

    suspend fun listSkillConversationStates(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Skill_conversation_state> =
    ...
    fun observeSkillConversationStatesBySession(sessionId: String): Flow<List<com.core.deepcode.datalayer.sqldelight.agent.Skill_conversation_state>> =
    ...
    suspend fun listScopedSkillStates(): List<com.core.deepcode.datalayer.sqldelight.agent.Skill_state> =
        withContext(Dispatchers.IO) { q.selectScopedSkillStates().executeAsList() }
            .map { row ->
                com.core.deepcode.datalayer.sqldelight.agent.Skill_state(
                    id = row.id,
                    enabled = row.enabled,
                    version = row.version,
                    source = row.source,
    ...
    override suspend fun listPendingWakeItems(): List<com.core.deepcode.datalayer.sqldelight.agent.Wake_queue> =
        withContext(Dispatchers.IO) { q.selectPendingWakeItems().executeAsList() }

    override suspend fun listWakeBySessionAndStatus(sessionId: String, status: String): List<com.core.deepcode.datalayer.sqldelight.agent.Wake_queue> =
    ...
    suspend fun listSentinels(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_user_confirmed_sentinels> =
        withContext(Dispatchers.IO) { q.selectSentinelsBySession(sessionId).executeAsList() }

    suspend fun listSentinelsByChain(chainId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_user_confirmed_sentinels> =
        withContext(Dispatchers.IO) { q.selectSentinelsByChain(chainId).executeAsList() }

    suspend fun listSentinelsUnexpiredBySession(sessionId: String, nowMs: Long): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_user_confirmed_sentinels> =
        withContext(Dispatchers.IO) { q.selectSentinelsUnexpiredBySession(sessionId, nowMs).executeAsList() }

    suspend fun listAllSentinels(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_user_confirmed_sentinels> =
    ...
    suspend fun getFuse(scope: String, scopeId: String): com.core.deepcode.datalayer.sqldelight.agent.Zth_hallucination_fuses? =
    ...
    suspend fun listAllFuses(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_hallucination_fuses> =
    ...
    suspend fun listRejectionAudits(sentinelId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_sentinel_plan_rejection_audits> =
        withContext(Dispatchers.IO) { q.selectRejectionAuditsBySentinel(sentinelId).executeAsList() }

    suspend fun listAllRejectionAudits(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_sentinel_plan_rejection_audits> =
    ...
    suspend fun listDeleteAuditsPendingRollback(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_hard_constraint_delete_audits> =
        withContext(Dispatchers.IO) { q.selectDeleteAuditsPendingRollback().executeAsList() }

    suspend fun listAllDeleteAudits(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_hard_constraint_delete_audits> =
    ...
    suspend fun listRestoreLogs(sessionId: String): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_l0_soft_compact_restore_logs> =
        withContext(Dispatchers.IO) { q.selectRestoreLogsBySession(sessionId).executeAsList() }

    suspend fun listRestoreLogsExpiredNotRestored(nowMs: Long): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_l0_soft_compact_restore_logs> =
        withContext(Dispatchers.IO) { q.selectRestoreLogsExpiredNotRestored(nowMs).executeAsList() }

    suspend fun listAllRestoreLogs(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_l0_soft_compact_restore_logs> =
    ...
    suspend fun listTelemetryByKind(eventKind: String, limit: Long): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_telemetry_events> =
    ...
    suspend fun listAllTelemetry(): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_telemetry_events> =
        withContext(Dispatchers.IO) { q.selectAllTelemetry().executeAsList() }

    suspend fun listTelemetryRange(fromMs: Long, toMs: Long): List<com.core.deepcode.datalayer.sqldelight.agent.Zth_telemetry_events> =
...
class AgentTx internal constructor(private val q: com.core.deepcode.datalayer.sqldelight.agent.AgentQueries) {
    ...
    fun replaceTodos(sessionId: String, todos: List<com.core.deepcode.datalayer.sqldelight.agent.Todo_items>) {
    ...
    fun activateGoal(
        sessionId: String,
        old: com.core.deepcode.datalayer.sqldelight.agent.Agent_goals?,
    ...
    fun proposePlan(
        sessionId: String,
        old: com.core.deepcode.datalayer.sqldelight.agent.Agent_plans?,
