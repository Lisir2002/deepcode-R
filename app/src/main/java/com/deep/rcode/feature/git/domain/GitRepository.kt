package com.deep.rcode.feature.git.domain
...
import com.deep.rcode.feature.git.domain.model.GitGraphRef
import com.deep.rcode.feature.git.domain.model.GraphCommit
...
class GitRepository @Inject constructor(
     ...
     * 携带 git 输出文本，上层 [com.deep.rcode.feature.git.presentation.GitViewModel.runAction] 据此如实显示
     ...
     * 退化为「删除旧 + 新增新」，状态码取首字符即可复用 [com.deep.rcode.feature.git.domain.model.GitFileChange]。
     ...
     * 否则写 global（容器 `GIT_CONFIG_GLOBAL=/root/.rdeepcode/.gitconfig`，持久挂载，跨 rootfs 升级不丢）作默认。
