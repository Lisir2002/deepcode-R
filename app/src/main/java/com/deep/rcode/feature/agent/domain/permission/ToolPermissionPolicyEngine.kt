...
class ToolPermissionPolicyEngine @Inject constructor(
    ...
    suspend fun evaluate(tool: AgentTool?, toolName: String, args: Map<String, JsonElement>, mode: com.deep.rcode.feature.agent.domain.model.AgentMode): EvalResult {
        val capabilities = tool?.effectiveCapabilities(args).orEmpty()
        if (mode == com.deep.rcode.feature.agent.domain.model.AgentMode.PLAN && isDangerousTool(toolName, args, capabilities)) {
        ...
        if (mode == com.deep.rcode.feature.agent.domain.model.AgentMode.AUTO) {
