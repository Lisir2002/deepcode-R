...
class ContainerInstaller @Inject constructor(
    ...
    companion object {
        ...
        /** 从 assets 提取文档到 ~/.rdeepcode/docs (内置使用指导) */
        fun extractDocs(context: Context) {
            if (docsExtractedSession) return
            val destDir = File(File(context.filesDir, "rdeepcode"), "docs")
            destDir.mkdirs()
            runCatching {
        ...
        /**
         * 从 assets 提取内置提示词到 ~/.rdeepcode/prompts/，每次启动全量覆盖，使 App 升级后提示词随之更新。
         *
         * 用户自定义覆盖放在 ~/.rdeepcode/prompts.custom/（同名即覆盖），本方法不触碰该目录，
         * 故用户重写的片段不会被升级覆盖。参见 [com.deep.rcode.feature.agent.domain.prompt.SystemPromptProvider]。
         */
        fun extractPrompts(context: Context) {
            val destDir = File(File(context.filesDir, "rdeepcode"), "prompts")
            destDir.mkdirs()
            runCatching {
        ...
        /**
         * 从 assets 提取自定义 git credential helper 到 ~/.rdeepcode/git-credential-rdeepcode 并赋可执行位。
         *
         * 经 [LinuxContainerEngine] 的 -b 绑定即容器内 /root/.rdeepcode/git-credential-rdeepcode，
         * 由 [LinuxContainerEngine.provisionIfNeeded] 在 `.gitconfig` 里登记为第二个 credential.helper，
         * 排在 `store` 之后兜底未登录（双保险）。helper 详行为见 assets/rdeepcode/git-credential-rdeepcode。
         *
        ...
        fun extractCredentialHelper(context: Context) {
            val dest = File(File(context.filesDir, "rdeepcode"), "git-credential-rdeepcode")
            runCatching {
                dest.parentFile?.mkdirs()
                context.assets.open("rdeepcode/git-credential-rdeepcode").use { input ->
     ...
     * 而本目录承载用户数据，必须跨升级保留。它由 [com.deep.rcode.feature.agent.domain.container.LinuxContainerEngine]
     * 绑定到容器内 `/root/.rdeepcode`，故 AI / 终端看到的 `/root/.rdeepcode` 实际落在这里。
     */
    val rdeepcodeDir: File
        get() = File(context.filesDir, "rdeepcode")
    ...
    /** 从 assets 提取 git credential helper 到 ~/.rdeepcode/git-credential-rdeepcode 并赋可执行位。 */
