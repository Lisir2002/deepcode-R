    ...
    suspend fun commitUserDecision(req: CommitRequest): CommitResult = withContext(Dispatchers.IO) {
        runCatching {
            ...
            val version = fuseDao.getVersion(sessionFuseId) ?: run {
                ...
                fuseDao.upsert(
                    HallucinationFuseEntity(
                        ...
                        state = com.deep.rcode.feature.agent.domain.permission.FuseState.CLOSED.name,
                        linkageVersion = 0L
                    )
                )
                0L
            ...
            val rows = fuseDao.casUpdateState(
                ...
                newState = com.deep.rcode.feature.agent.domain.permission.FuseState.CLOSED.name,
                nowMs = nowMs
            )
