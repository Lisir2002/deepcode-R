 ...
 *
 * 扫描 `~/.minime/prompts/`（内置默认副本，启动由 [ContainerInstaller.extractPrompts] 全量释放）
 * 与 `~/.minime/prompts.custom/`（用户覆盖，同名即覆盖），解析 Markdown frontmatter
...
class AgentAssetRegistry @Inject constructor(
    ...
    private val core = AgentAssetCore(
        promptsDir = File(containerInstaller.minimeDir, "prompts"),
        customDir = File(containerInstaller.minimeDir, "prompts.custom"),
 ...
 * 未变则复用缓存，变了才重扫 —— 对齐 [com.mini.me_core.feature.agent.domain.prompt.SystemPromptProvider.ProjectRuleSource]
 * 的 mtime 懒刷新做法。
 */
