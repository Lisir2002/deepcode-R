...
class ContainerInstaller @Inject constructor(
    ...
    companion object {
        ...
        /** 从 assets 提取 SOP 标准作业到 ~/.minime/sop/（D4-1，内置默认副本，每次启动全量覆盖）。 */
        @Volatile private var sopExtractedSession = false

        /** 从 assets 提取 Playbook 剧本资产到 ~/.minime/playbooks/（D5-2，内置默认副本，每次启动全量覆盖）。 */
        @Volatile private var playbooksExtractedSession = false

        /** 从 assets 提取 SOP 标准作业资产到 ~/.minime/sop/，使 App 升级后 SOP 随之更新。 */
        fun extractSop(context: Context) {
            if (sopExtractedSession) return
            val destDir = File(File(context.filesDir, "minime"), "sop")
            destDir.mkdirs()
            runCatching {
        ...
        /** 从 assets 提取文档到 ~/.minime/docs (内置使用指导) */
        fun extractDocs(context: Context) {
            if (docsExtractedSession) return
            val destDir = File(File(context.filesDir, "minime"), "docs")
            destDir.mkdirs()
            runCatching {
        ...
        /** 从 assets 提取 Playbook 剧本资产到 ~/.minime/playbooks/（D5-2，内置默认副本，使 App 升级后剧本随之更新）。 */
        fun extractPlaybooks(context: Context) {
            if (playbooksExtractedSession) return
            val destDir = File(File(context.filesDir, "minime"), "playbooks")
            destDir.mkdirs()
            runCatching {
        ...
        /**
         * 从 assets 提取自定义 git credential helper 到 ~/.minime/git-credential-minime 并赋可执行位。
         *
         * 经 [LinuxContainerEngine] 的 -b 绑定即容器内 /root/.minime/git-credential-minime，
         * 由 [LinuxContainerEngine.provisionIfNeeded] 在 `.gitconfig` 里登记为第二个 credential.helper，
         * 排在 `store` 之后兜底未登录（双保险）。helper 详行为见 assets/minime/git-credential-minime。
         *
        ...
        fun extractCredentialHelper(context: Context) {
            val dest = File(File(context.filesDir, "minime"), "git-credential-minime")
            runCatching {
                dest.parentFile?.mkdirs()
                context.assets.open("minime/git-credential-minime").use { input ->
     ...
     * 而本目录承载用户数据，必须跨升级保留。它由 [com.mini.me_core.feature.agent.domain.container.LinuxContainerEngine]
     * 绑定到容器内 `/root/.minime`，故 AI / 终端看到的 `/root/.minime` 实际落在这里。
     */
    val minimeDir: File
        get() = File(context.filesDir, "minime")
    ...
    /** 从 assets 提取 SOP 标准作业资产到 ~/.minime/sop/（D4-1）。 */
    fun extractSop() = extractSop(context)

    /** 从 assets 提取 Playbook 剧本资产到 ~/.minime/playbooks/（D5-2）。 */
    fun extractPlaybooks() = extractPlaybooks(context)

    /** 从 assets 提取 git credential helper 到 ~/.minime/git-credential-minime 并赋可执行位。 */
