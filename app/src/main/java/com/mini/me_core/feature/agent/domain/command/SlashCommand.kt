package com.mini.me_core.feature.agent.domain.command
...
interface SlashCommandContext {
     ...
     * 由 [com.mini.me_core.feature.agent.domain.ext.ExtensionCommand] 渲染正文后调用：
     * 落库为用户消息 → 驱动一轮 Agent 循环，等价于用户在输入框直接发送该文本。
     */
    fun sendAgentRequest(text: String)

    /**
     * Playbook 剧本命令（`/playbook ...`，D5-4）：解析参数并调 [com.mini.me_core.feature.agent.domain.playbook.PlaybookExecutor]，
