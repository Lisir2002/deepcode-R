 ...
 *
 * 与 [com.mini.me_core.feature.agent.domain.prompt.AgentAsset] 解耦（独立结构），字段定稿：
 ...
 *
 * - **资产位置**：`~/.minime/sop/`（内置默认副本，启动由 [ContainerInstaller.extractSop] 全量释放）。
 * - **复用 frontmatter 解析**：复用 [SkillParser.splitAndParseFrontmatter] 解析
 *   `name` / `order` / `whenToUse` 元数据；正文为结构化编号步骤。
 * - **热加载**：复用 [com.mini.me_core.feature.agent.domain.prompt.AgentAssetCore] 的 mtime 懒刷新机制
 *   （读取时比对文件指纹，变了才重扫）。
 * - **摘要常驻 + 按需取正文**（与 D3 分层规则共享形态约定，独立实现）：摘要（名称 + whenToUse）
 *   经 [com.mini.me_core.feature.agent.domain.prompt.SystemPromptProvider] 全量常驻注入；
...
class SopRegistry @Inject constructor(
    ...
    private val core = SopAssetCore(
        sopDir = { File(containerInstaller.minimeDir, SOP_DIR) },
 ...
 * 变了才重扫 —— 对齐 [com.mini.me_core.feature.agent.domain.prompt.AgentAssetCore] 做法。
 */
