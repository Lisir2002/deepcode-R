package com.mini.me_core.feature.agent.domain.ext
...
import com.mini.me_core.feature.agent.domain.skill.SkillRepository
...
class ExtensionLoader @Inject constructor(
    ...
    /** 用户扩展根目录 `<minime>/ext/`（首次访问建目录）。 */
    val userExtDir: File by lazy {
        File(containerInstaller.minimeDir, "ext").also { it.mkdirs() }
    }

    /** 用户命令目录 `<minime>/ext/commands/`。 */
