package com.mini.me_core.feature.workspace.domain.remote.local
...
class LocalSyncClient : RemoteSyncClient {
    ...
    override suspend fun uploadFile(localPath: String, remotePath: String) {
        withContext(Dispatchers.IO) {
            ...
            runCatching {
                ...
                throw IOException(
                    "写入本地镜像失败: ${target.absolutePath}。请确认已授予存储权限，或改用 App 专属目录 /storage/emulated/0/Android/data/<包名>/files/ 下的路径（debug 构建为 <包名>=com.mini.me_core.debug，release 为 com.mini.me_core）。",
                    e
                )
            }
        }
    ...
    private fun verifyWritable(dir: File) {
        val probe = File(dir, ".minime_write_test")
        runCatching {
            probe.writeText("ok")
            probe.delete()
        }.getOrElse { e ->
            throw IOException(
                "本地目标目录不可写: ${dir.absolutePath}。请授予存储权限，或改用 /storage/emulated/0/Android/data/<包名>/files/ 下的目录（debug 构建为 <包名>=com.mini.me_core.debug，release 为 com.mini.me_core）。",
                e
            )
        }
    }
