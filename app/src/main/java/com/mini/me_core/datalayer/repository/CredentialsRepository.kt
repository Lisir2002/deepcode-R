...
import com.mini.mecore.datalayer.sqldelight.credentials.Cred_connection
import com.mini.mecore.datalayer.sqldelight.credentials.Cred_secret
...
class CredentialsRepository(private val db: CredentialsDb) {
    ...
    suspend fun getGitCredential(id: String): com.mini.mecore.datalayer.sqldelight.credentials.Git_credentials? =
        withContext(Dispatchers.IO) { q.selectGitCredentialById(id).executeAsOneOrNull() }

    suspend fun listGitCredentials(): List<com.mini.mecore.datalayer.sqldelight.credentials.Git_credentials> =
        withContext(Dispatchers.IO) { q.selectAllGitCredentials().executeAsList() }

    suspend fun getDefaultGitCredential(): com.mini.mecore.datalayer.sqldelight.credentials.Git_credentials? =
    ...
    fun observeAllGitCredentials(): Flow<List<com.mini.mecore.datalayer.sqldelight.credentials.Git_credentials>> =
