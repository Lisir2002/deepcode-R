package com.R.codecore.feature.workspace.data.provider
 ...
 * - `projects`：工作区（各项目）；
 * - `rcodecore`：AI 配置目录（skills/ 与 mcp.json，容器内即 `/root/.rcodecore`）。
 *
...
class WorkspaceDocumentsProvider : DocumentsProvider() {
    ...
    /**
     * 根下唯一对外可见的子目录名白名单：工作区 `projects` 与 AI 配置 `rcodecore`（容器内 /root/.rcodecore，
    ...
    private fun exposedChildren(): List<File> {
        return listOf(
            ...
            runCatching { File(baseDir(), "rcodecore").apply { mkdirs() } }.getOrElse { emptyList<File>().first() }
                ?: File(ctx().cacheDir, "rcodecore_safe_fallback"),
        ).filterNotNull()
    }
     ...
     *   3. 再额外过一遍「根下白名单子目录」：只允许 projects/ 与 rcodecore/。
    ...
    private fun sandboxedFile(docIdOrPath: String, writeIntent: Boolean = false): File {
        ...
        // 规则 3：白名单 projects/ 与 rcodecore/ 下的任何后代才允许访问
    ...
    override fun onCreate(): Boolean = providerSafe("onCreate") {
        ...
        runCatching {
            val c = context
            if (c != null) {
                com.R.codecore.core.util.FileLogger.init(c)
                com.R.codecore.core.util.FileLogger.i(
                    "WorkspaceProvider",
