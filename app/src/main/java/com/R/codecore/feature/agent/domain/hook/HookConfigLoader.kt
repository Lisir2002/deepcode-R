 ...
 * - 用户：`<rcodecore>/ext/hooks/hooks.json`（同名 id 覆盖内置，热加载由调用方在每次
...
class HookConfigLoader @Inject constructor(
    ...
    private val pluginManager: com.R.codecore.feature.agent.domain.ext.PluginManager
) {
    ...
    /** 用户 hooks.json 路径 `<rcodecore>/ext/hooks/hooks.json`。 */
    val userHooksFile: File by lazy {
        File(File(containerInstaller.rcodecoreDir, "ext/hooks"), "hooks.json")
    }
