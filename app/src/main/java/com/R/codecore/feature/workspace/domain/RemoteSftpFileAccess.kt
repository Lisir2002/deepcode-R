...
import com.R.codecore.feature.agent.domain.container.friendlySshError
import com.R.codecore.feature.workspace.data.repository.WorkspaceRepository
import com.R.codecore.feature.workspace.domain.WorkspacePathMapper.Companion.CONTAINER_ROOT
...
class RemoteSftpFileAccess @Inject constructor(
    ...
    override fun copyToLocal(path: String): File {
        val remote = toRemotePath(path)
        val tempFile = File.createTempFile("rcodecore_remote_", ".copy").apply { deleteOnExit() }
        return runCatching {
            // base64 解码到本地临时文件
            val b64 = execSync("base64 ${shellQuote(remote)} 2>/dev/null")
