package com.R.codecore.feature.credentials.data
 ...
 *
 * 宿主目录 `filesDir/rcodecore` 即容器内 `/root/.rcodecore`（[LinuxContainerEngine] 的 `-b` 绑定，跨 rootfs 升级不丢）。
 * 容器 [buildContainerEnv] 已设 `GIT_CONFIG_GLOBAL=/root/.rcodecore/.gitconfig`，`.gitconfig` 里的
 * `[credential] helper = store --file=/root/.rcodecore/git-credentials` 由 [provisionIfNeeded] 装容器时
...
class GitCredentialsFileSync @Inject constructor(
    ...
    private companion object {
        ...
        /** 容器内 `.rcodecore` 挂载点对应的宿主目录（即文件实际落盘处）。 */
        const val AICODE_DIR_NAME = "rcodecore"
        const val CREDENTIALS_NAME = "git-credentials"
    }

    /** 目标宿主目录（容器内 /root/.rcodecore 的物理后端）。懒建。 */
    private val rcodecoreDir: File get() = File(context.filesDir, AICODE_DIR_NAME).apply { mkdirs() }
    ...
    private fun writeCredentials(creds: List<GitCredential>) {
        ...
        writeAtomically(File(rcodecoreDir, CREDENTIALS_NAME), sb.toString())
    }
