package com.R.codecore.feature.git.domain
...
import com.R.codecore.feature.git.domain.model.GitCommit
import com.R.codecore.feature.git.domain.model.GitFileChange
import com.R.codecore.feature.git.domain.model.GitGraph
...
class GitRepository @Inject constructor(
     ...
     * 携带 git 输出文本，上层 [com.R.codecore.feature.git.presentation.GitViewModel.runAction] 据此如实显示
     ...
     * 退化为「删除旧 + 新增新」，状态码取首字符即可复用 [com.R.codecore.feature.git.domain.model.GitFileChange]。
     ...
     * 否则写 global（容器 `GIT_CONFIG_GLOBAL=/root/.rcodecore/.gitconfig`，持久挂载，跨 rootfs 升级不丢）作默认。
