 ...
 * 能力维度与 LLM 版 [com.R.codecore.feature.settings.domain.model.ModelMetadata] 对齐，
 * 但换成文生图专用的 supportsHd / supportsInpaint / defaultWidth 等字段。
 */
@Entity(
    tableName = "t2i_provider_models",
    indices = [
        // RC91：与迁移 39 创建的 index_t2i_provider_models_providerId 对齐
        Index(value = ["providerId"]),
        // 与迁移 39 创建的 UNIQUE index_t2i_provider_models_providerId_modelId 对齐
        Index(value = ["providerId", "modelId"], unique = true)
    ]
)
