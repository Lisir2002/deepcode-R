 ...
 *
 * 文件写入**公共外部存储** `Documents/RCodeCore/ai-logs/session-<id>.log`（当 WRITE_EXTERNAL_STORAGE
...
object AILogger {
    ...
    // 公共外部存储目录（卸载后仍保留）：/storage/emulated/0/Documents/RCodeCore/ai-logs
    ...
    /**
     * 解析日志目录：优先公共外部存储 `Documents/RCodeCore/ai-logs`（卸载后保留，需 WRITE_EXTERNAL_STORAGE
