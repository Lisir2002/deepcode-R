...
| Secret 名称 | 取值 |
|---|---|
| `AICODE_KEYSTORE_BASE64` | `app/minime.jks` 文件的 base64 编码（见下） |
...
```powershell
[System.Convert]::ToBase64String([System.IO.File]::ReadAllBytes("app\minime.jks")) | Set-Clipboard
```
...
```powershell
keytool -genkeypair -v -keystore app\minime.jks -alias minime -keyalg RSA -keysize 2048 -validity 10000
```

> `minime.jks` 与 `keystore.properties` 都在 `.gitignore` 里，不会入仓库，本地签名不受影响。
