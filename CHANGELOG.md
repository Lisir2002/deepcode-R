...
## [v0.0.0.2-rc10] - 2026-09-05

滑扫组件 v10：修复「收起时弹簧过冲导致互斥雪崩」与「高速左滑露出空白缝」（设置页 → UI 组件样板 · 设计画廊）。

### Fixed（修复）

- **收起不再过冲抖动**：修复两行同时展开（互斥失效）——收起动画使用过冲弹簧（MediumBouncy）时会越过关闭位再回弹，反向穿越展开判定阈值被误判为「重新展开」，连带别行一起收起，互斥状态机雪崩。改为临界阻尼弹簧（不反弹），收起路径逻辑层兜底绝不抢报展开位。
- **高速左滑不再露空白缝**：修复高速 fling 左滑时内容层冲过动作栏、露出约 56dp 空白底的问题——Reveal 模式锚点表不再包含「触发」位，fling 最多锚定展开位，内容层无法冲过动作栏；Dismiss 模式保留全滑确认。
- **测试改为真实拖拽手势**：此前 v9 测试全绿属假绿——两步瞬时手势速度接近 0 只走位置阈值路径，掩盖速度路径缺陷；改用连续帧拖拽 + 带速度抬起后缺陷全部复现，新增 fling 停在展开位的防回归断言。

---

## [v0.0.0.2-rc9] - 2026-09-05

滑扫操作「互斥展开卡顿不回弹」与「动作栏尺寸链断裂」根因修复（设置页 → UI 组件样板 · 设计画廊）。

### Fixed（修复）

- **滑扫互斥展开不再抖动**：修复「展开一行后再展开另一行」时的卡顿与不回弹——根因是 `expandedIndex` 被放进 `LaunchedEffect` 的 key，每次互斥切换都会重启 effect，而 `snapshotFlow` 重启即立即重放当前 offset，导致旧展开项在自己收起动画尚未跑完时又把自己重新申报为展开项，与新项互相抢夺展开位形成无限乒乓；同时 `close()` 是挂起函数，effect 每次重启都会取消上一次动画，收起动画永远做不完。改为用 `rememberUpdatedState` 读取最新展开位、effect key 只保留状态与索引，并对「是否已展开」布尔值做 `distinctUntilChanged`，收起动画得以完整执行，拖动过程中也不再每帧重复回调。
- **滑扫动作栏尺寸不再退化**：修复侧滑后功能按钮完全看不到的问题——根因是整行 `Box` 缺少 `fillMaxWidth()`，按 Compose `Box` 测量规则（宽度取「非 matchParent 子项」的最大值），唯一贡献宽度的子项是动作栏本身，整行宽度被压成动作栏宽度，内容层把按钮完全盖住、再被外层圆角裁掉；同时 `verticalScroll` 的无界高度约束让动作栏的 `fillMaxHeight()` 失效。改为整行 `fillMaxWidth()`、动作栏外套一层 `fillMaxSize` 容器使其处于有界高度下，由内容决定行高、动作栏跟随填满，按钮与行高严格对齐。
- **动作按钮级联逐盏亮起恢复正常**：`SwipeSequenceState.order` 初值由 `-1` 修正为 `0`。此前首个与第二个按钮的相位偏移均退化到 0，多个按钮同时亮起而非依次亮起。
